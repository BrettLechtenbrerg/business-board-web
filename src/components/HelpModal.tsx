...
const steps = [
  ...
  },
  {
    ...
    content: [
      {
        subtitle: 'Pick Your Business Mentor',
        instructions: [
          'Click on any advisor card to select them',
          'Each advisor has a unique business philosophy',
          'Alex Hormozi for offers and scaling',
          'Mark Cuban for deals and execution',
          'Gary Vee for marketing and personal brand',
  ...
  },
  {
    ...
    content: [
      {
        subtitle: 'Ask Anything About Business',
        instructions: [
          'Type your question in the chat box',
          'Ask about strategy, marketing, scaling, or any business topic',
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] overflow-hidden rounded-2xl border border-white/10 bg-gray-900/95 shadow-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-gray-900/95 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-white/50 mt-0.5">Set up your Business Board of Advisors</p>
          </div>
          <button onClick={onClose} className="rounded-lg p-2 text-white/50 hover:text-white hover:bg-white/10 transition-colors">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto p-6 space-y-3" style={{ maxHeight: 'calc(85vh - 140px)' }}>
          {steps.map((step) => {
            ...
            return (
              <div key={step.number} className="rounded-xl border border-white/10 bg-white/[0.02] overflow-hidden">
                <button
                  onClick={() => setExpandedStep(isExpanded ? null : step.number)}
                  className="w-full flex items-center gap-4 p-4 hover:bg-white/[0.03] transition-colors"
                >
                  <div className={`flex items-center justify-center w-10 h-10 rounded-xl border ${colorClasses[step.color] || colorClasses.purple}`}>
                  ...
                  <ChevronRight className={`h-5 w-5 text-white/40 transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`} />
                </button>
                      ...
                      <div key={idx}>
                        {(section as { isWarning?: boolean }).isWarning ? (
                          <div className="flex gap-3 p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                            ...
                            <p className="text-sm text-amber-200/90 leading-relaxed">{section.instructions[0]}</p>
                          </div>
                          ...
                          <>
                            {section.subtitle && (
                              <h4 className="text-sm font-medium text-white/70 mb-2 ml-14">{section.subtitle}</h4>
                            )}
                            ...
                            {(section as { link?: { url: string; label: string } }).link && (
                              <a
                                ...
                                className="inline-flex items-center gap-1.5 ml-14 mt-2 text-sm text-purple-400 hover:text-purple-300 transition-colors font-medium"
                              >
          ...
          <div className="pt-4 border-t border-white/10">
            ...
            <div className="flex flex-wrap gap-2">
              <a href="https://platform.claude.com/dashboard" target="_blank" rel="noopener noreferrer"
              ...
              <a href="https://www.youtube.com/watch?v=y3Jx8sIwYQs" target="_blank" rel="noopener noreferrer"
                ...
                <PlayCircle className="h-4 w-4" /> Video Tutorial
              </a>
              <a href="https://docs.anthropic.com/en/docs/quickstart" target="_blank" rel="noopener noreferrer"
                ...
                <ExternalLink className="h-4 w-4" /> Anthropic Docs
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="sticky bottom-0 border-t border-white/10 bg-gray-900/95 px-6 py-4">
          <button onClick={onClose} className="w-full py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-violet-500 text-white font-medium hover:from-purple-500 hover:to-violet-400 transition-all">
            Got it, let&apos;s build!
          </button>
        </div>
      </div>
